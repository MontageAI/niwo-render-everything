...
文案定稿后，从文案里提炼竖屏顶部显示的**钩子大标题**，写进 `content.json` 的 `hook_headline`。它不进配音、不进字幕，只负责开场抓注意力。

- 推荐 2 行：第一行铺垫，第二行给结果或冲突。
- 每行不超过 12 字，把最有冲击力的数字或结论用 `[[ ]]` 圈出来做高亮。
- 竖屏 `9:16` 优先填写；横屏 `16:9` 或方形 `1:1` 可省略。
...
只有 `title`、`script`、`aspect_ratio`、`hook_headline`、`pronunciations` 和 `notes` 这几项，全都是内容。这份 JSON 不接受任何未列出的字段，把渲染参数写进去会直接校验失败。
...
- [ ] `hook_headline`（如有）为 1 到 3 行、每行 12 字内、至少一处 `[[ ]]` 高亮
