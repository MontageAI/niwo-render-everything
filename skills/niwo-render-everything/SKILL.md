...
你只负责**内容**：文案、成片方向、素材、多音字读音、钩子标题。音色、语速、字幕开关、IP 形象、背景音乐这些渲染参数由用户在 Niwo 里渲染前自己选，不要写进 `content.json`。
...
- 竖屏视频 `portrait`（默认）：内容铺满 9:16 画布
- 横屏视频 `landscape`：内容铺满 16:9 画布
- 竖屏信息版 `portrait_editorial`：成片是 9:16，内容按 16:9 横屏渲染后嵌进画布中部，钩子标题在上方、字幕与 IP 形象在下方

用户没回答、或让你自己定，就按竖屏 `portrait`。确认结果要原样写进 `content.json` 的 `video_format`。

形态决定素材取向：`portrait` 收竖构图，`landscape` 与 `portrait_editorial` 都收横构图。
...
只有 `portrait_editorial` 需要。文案定稿后，从文案里提炼那条顶部标题带上的**钩子大标题**，写进 `content.json` 的 `hook_headline`。它不进配音、不进字幕，只负责开场抓注意力。
...
- `landscape` 与 `portrait` 没有这块版面，一律省略此字段。
...
- [ ] `video_format` 与第零步确认的成片形态一致（未确认则为 `portrait`）
